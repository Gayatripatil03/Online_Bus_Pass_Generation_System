...
<head>
    ...
    if(isset($_POST['change']))
    {
        ...
        $chpass=mysqli_query($connect,"select * from admin where Email='".$_SESSION['email']."' and Password='".$_POST['oldpass']."'") or die(mysqli_error($connect));
        if(mysqli_num_rows($chpass)==1)
        {
            if(strlen($_POST['newpass'])>=3)
            {
                ...
                {
                    $update=mysqli_query($connect,"update admin set Password='".$_POST['newpass']."' , 
                            Password='".$_POST['repass']."' where Email='".$_SESSION['email']."' " )or die(mysqli_error($connect));
                    if($update)
                    {
                        echo "<script>";
